  ...
  device_sensors = flatten([
    for device in local.devices : [
      for dev_sensor in [try(local.device_config[device.name].device_sensor, null)] : {
        ...
        filter_lists_dhcp = [for dhcp_filter in try(dev_sensor.dhcp_filter_lists, []) : {
        ...
        filter_lists_lldp = [for lldp_filter in try(dev_sensor.lldp_filter_lists, []) : {
        ...
        # filter_lists_cdp = [for cdp_filter in try(dev_sensor.cdp_filter_lists, []) : {
